...
const p2pLengthVal = document.getElementById('p2pLengthVal');
...
// ---- Count / Creativity segmented buttons ----
function wireChoiceRow(rowId, hiddenInput) {
  document.querySelectorAll(`#${rowId} .p2p-choice-btn`).forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      hiddenInput.value = btn.dataset.count || btn.dataset.creativity;
    });
  });
}
...
p2pLength.addEventListener('input', () => {
  p2pLengthVal.textContent = `${p2pLength.value} words`;
});
...
['dragenter', 'dragover'].forEach(evt =>
  p2pImageGrid.addEventListener(evt, (e) => { e.preventDefault(); p2pImageGrid.classList.add('drag-active'); })
);
...
function cardHtmlP2p(p, i) {
  return `
    <div class="p2p-card pop-in">
      ...
      <div class="p2p-card-text">${p.replace(/</g, '&lt;')}</div>
